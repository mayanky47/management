...
export const useProjects = () => {
    ...
    const fetchProjects = useCallback(async () => {
        ...
        setError(null);
        try {
            const data = await makeApiCall(`${API_BASE_URL}/plan-projects`, { method: 'GET' });
            setProjects(data || []);
        } catch (err) {
    ...
    const saveProject = async (projectData) => {
        ...
        const url = isUpdate ? `${API_BASE_URL}/plan-projects/${projectData.id}` : `${API_BASE_URL}/plan-projects`;
    ...
    const deleteProject = async (id) => {
        try {
            await makeApiCall(`${API_BASE_URL}/plan-projects/${id}`, { method: 'DELETE' });
